...
export async function configent(options) {
  const cfg = new Configent(options, 3);
...
class Configent {
  ...
  async getDetectDefaults() {
    const { name, consumerDir } = this.options;
    const hash = JSON.stringify({ name, consumerDir });
    ...
    if (!detectedFromDefaults[hash] || !this.options.cacheDetectedDefaults) {
      ...
      if (existsSync("package.json"))
        Object.assign(pkgjson, JSON.parse(readFileSync(resolve(this.options.consumerDir, "package.json"), "utf-8")));
      ...
      const unsortedConfigTemplates = await Promise.all(
        readdirSync(defaultConfigsPath).map(async (file) => ({
          file,
